...
import Layot from '../Shared/OtherLayout';
...
const Dealer = () => {
  ...
  const allDealers = Array.isArray(dealers?.data) ? dealers.data : [];

  //filter dealers

   useEffect(() => {
    if (!userInfo?.email || !allDealers.length) return;

    // Find dealer by the logged-in user's email
    const myDealer = allDealers.find((u) => u.email === userInfo.email);
    ...
    if (myDealer?._id) {
      ...
      setDId(myDealer._id);
      console.log("Filtered dealer:", myDealer);
    } else {
