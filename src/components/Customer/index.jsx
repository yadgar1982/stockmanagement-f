...
import OtherLayout from '../Shared/OtherLayout';
...
const Statements = () => {
  ...
  useEffect(() => {
      if (!userInfo?.email || !allCustomers.length) return;
  
      // Find dealer by the logged-in user's email
      const myCustomer = allCustomers.find((u) => u.email === userInfo.email);
 
    
      if (myCustomer?._id) {
  
        setCId(myCustomer._id); // save dealer ID for filtering later
        console.log("Filtered dealer:", myCustomer);
      } else {
        console.log("Dealer not found for this user");
      }
    }, [userInfo, allCustomers]);
 
 

  // Prepare customer purchase/payment data
  useEffect(() => {
    if (!cId) return;
  ...
  }, [cId, selectedCurrency, dateRange, statementWithBalance]);
  ...
  ">
    <header style="width: 100%; text-align: center; margin-bottom: 20px;">
      ...
      <div style="text-align:left; margin-top:5px;">
        ...
        <div style="font-size:12px; color:#5a5b5cff;"><strong>Name:</strong> ${myCustomerData.fullname}</div>
        <div style="font-size:12px; color:#5a5b5cff;"><strong>Account No:</strong> ${myCustomerData.accountNo}</div>
        <div style="font-size:12px; color:#5a5b5cff;"><strong>Mobile:</strong> ${myCustomerData.mobile}</div>
    ...
    <OtherLayout>
      ...
      <div className='px-2'>
        <h1 className="md:text-xl text-sm px-2 !mb-3 text-yellow-700 font-extrabold " style={{ fontFamily: 'Poppins, sans-serif' }}>
         <span className='!text-cyan-500'> {userInfo?.fullname}</span> Financial Statements:
        </h1>
